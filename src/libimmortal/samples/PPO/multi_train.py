...
def train(args):
    ...
    # ------------------------------------------------------------
    # Rollout parallelism sizing
    # ------------------------------------------------------------
    # Convention:
    #   - args.n_envs: total envs across ALL ranks (default)
    #   - args.n_envs_per_rank: override per-rank env count (optional)
    total_envs = int(getattr(args, "n_envs", 1))
    total_envs = max(1, total_envs)

    n_envs_per_rank_override = getattr(args, "n_envs_per_rank", None)
    if n_envs_per_rank_override is not None:
        n_envs = int(n_envs_per_rank_override)
    else:
        # Distribute total envs across ranks (ceil => may create a few extra envs overall)
        n_envs = int(math.ceil(total_envs / float(max(1, world))))
    n_envs = max(1, n_envs)

    # Per-rank current observations (filled after per-env reset/bootstrap)
    cur_id_maps = [None] * n_envs  # List[Optional[np.ndarray]]; each expected shape (H, W)
    cur_vecs    = [None] * n_envs  # List[Optional[np.ndarray]]; each expected shape (D,)

    if ddp.is_main_process():
        print(f"[Rollout] world_size={world} total_envs={total_envs} -> n_envs_per_rank={n_envs}", flush=True)
    ...
    # IMPORTANT: for port uniqueness, we only need uniqueness within a node.
    # torchrun sets LOCAL_WORLD_SIZE; fallback to world_size for single-node runs.
    local_world = int(os.environ.get("LOCAL_WORLD_SIZE", str(world)))
    slots_per_node = int(local_world) * int(n_envs)

    # English comments for copy/paste friendliness.
    n_envs = int(getattr(args, "n_envs", 1))
    slots_per_node = int(world) * int(n_envs)  # assuming 1 node torchrun; if multi-node, include node_rank too.

    def _compute_port_for(env_i: int, restart_id: int) -> int:
        slot_id = int(local_rank) * int(n_envs) + int(env_i)
        return (
            int(args.port)
            + slot_id * int(port_stride)
            + int(restart_id) * int(port_stride) * int(slots_per_node)
        )

        # Build env pool
    envs = [None] * n_envs
    ports = [None] * n_envs
    restart_ids = [0] * n_envs  # per-env restart counter

    def _make_env_i(i: int):
        p = _compute_port_for(i, restart_ids[i])
        kw = dict(
            game_path=args.game_path,
            port=p,
            time_scale=args.time_scale,
            seed=(int(args.seed) + rank * 1000 + env_i) if args.seed is not None else None,
            width=args.width,
            height=args.height,
    ...
    def _restart_env_i(i: int, reason: str):
        restart_ids[i] += 1
        if restart_ids[i] > max_env_restarts:
            raise RuntimeError(f"[Env] env#{i} exceeded max_env_restarts={max_env_restarts} reason={reason}")
        try:
            if envs[i] is not None:
                envs[i].close()
        except Exception:
            pass
        envs[i], ports[i] = _make_env_i(i)
        return envs[i], ports[i]
    
    from concurrent.futures import ThreadPoolExecutor

    step_pool = ThreadPoolExecutor(max_workers=n_envs)

    def _step_env_i(i: int, action_env):
        # This runs in a worker thread.
        return envs[i].step(action_env)
    ...
    def _select_action_batch(id_maps_np, vecs_np):
        # id_maps_np: list[np.ndarray] length n_envs, each (H,W)
        # vecs_np:    list[np.ndarray] length n_envs, each (D,)
        map_t = torch.from_numpy(np.stack(id_maps_np, axis=0)).to(model_device, dtype=torch.long)
        vec_t = torch.from_numpy(np.stack(vecs_np, axis=0)).to(model_device, dtype=torch.float32)

        with torch.no_grad():
            # Expect batched outputs: (N, ...). If your act() only supports (1, ...),
            # fallback to per-env loop.
            action_t, logp_t, value_t = ddp.get_module(ppo_agent.policy_old).act(map_t, vec_t)

        a_cpu = action_t.detach().cpu()
        lp_cpu = logp_t.detach().cpu()
        v_cpu = value_t.detach().cpu()
        action_np = a_cpu.numpy()
        return a_cpu, lp_cpu, v_cpu, action_np
